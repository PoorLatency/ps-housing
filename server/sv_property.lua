...
function Property:RemoveFromDoorbellPool(src)
	...
	if self.playersDoorbell[_src] then self.playersDoorbell[_src] = nil end
...
lib.callback.register("ps-housing:cb:getPropertyInfo", function(source, property_id)
	...
	if ownerCid then
		ownerPlayer = QBCore.Functions.GetPlayerByCitizenId(ownerCid)
			or QBCore.Functions.GetOfflinePlayerByCitizenId(ownerCid)
