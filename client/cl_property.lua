...
function Property:RegisterGarageZone()
	...
	if not (self.has_access or self.owner) or not self.owner then return end
...
function Property:GiveMenus()
	...
	if self.owner or accessAndConfig then
		...
		end, "ps-housing:client:openFurnitureMenu", { propertyId = self.property_id })
	end

	if self.owner then
		Framework[Config.Radial].AddRadialOption("access_menu", "Manage Property", "key", function()
            self:ManageAccessMenu()
		end, "ps-housing:client:openManagePropertyAccessMenu", { propertyId = self.property_id })
	end
end
...
function Property:OpenDoorbellMenu()
	...
	for k, v in pairs(self.doorbellPool) do
		menu.options[#menu.options + 1] = {
			title = v.name,
			onSelect = function()
				TriggerServerEvent("ps-housing:server:doorbellAnswer", { targetSrc = v.src, property_id = self.property_id })
			end,
		}
	end
