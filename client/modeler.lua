...
local function isInside(coords)
	...
	if isX and isY and isZ then return true end
...
AddEventHandler("freecam:onTick", function()
	...
	if Modeler.CurrentCameraPosition and Modeler.CurrentCameraLookAt then
		...
		if posX and posY and posZ and lookAtX and lookAtY and lookAtZ then return end
	end
	...
	SendNUIMessage({
		action = "updateCamera",
		data = { cameraPosition = Modeler.CurrentCameraPosition, cameraLookAt = Modeler.CurrentCameraLookAt },
	})
end)
...
Modeler = {
	...
	OpenMenu = function(self, property_id)
		...
		SendNUIMessage({ action = "setFurnituresData", data = Config.Furnitures })
	...
	CloseMenu = function(self)
		...
		SendNUIMessage({ action = "setOwnedItems", data = {} })
	...
	FreecamMode = function(self, bool)
		if bool then --not in UI
			...
			SetNuiFocus(true, true)
			SendNUIMessage({ action = "freecamMode", data = false })
		end
	end,
	...
	StartPlacement = function(self, data)
		...
		SendNUIMessage({ action = "setObjectAlpha", data = self.CurrentObjectAlpha })
	...
	MoveObject = function(self, data)
		...
		if not isInside(coords) then return end

        ---@diagnostic disable-next-line: missing-parameter
		SetEntityCoords(self.CurrentObject, coords) -- get the current offset of this object in relation to the (?)
	end,
	...
	UpdateFurnitures = function(self)
		if not self.IsMenuActive then return end
		...
		SendNUIMessage({ action = "setOwnedItems", data = property.furnitureObjs })
	end,
	...
	AddToCart = function(self, data)
		...
		SendNUIMessage({ action = "addToCart", data = item })
	...
	RemoveFromCart = function(self, data)
		...
		if item ~= nil then
			...
			SendNUIMessage({ action = "removeFromCart", data = item })
	...
	UpdateCartItem = function(self, data)
		...
		if item ~= nil then item = data end
	end,
	...
	RemoveOwnedItem = function(self, data)
		...
		if item ~= nil then
			SendNUIMessage({ action = "removeOwnedItem", data = item })
